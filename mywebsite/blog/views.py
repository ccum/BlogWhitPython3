...
def month(request, year, month):
    ...
    return render_to_response("listado.html",dict(entrada_list=entrada,usuario=request.user,month=mkmonth_lst(),archive=True))

def entrada(request,pk):
    identrada=Entrada.objects.get(pk=int(pk))
    comentarios=Comentario.objects.filter(identrada = identrada)
    d= dict(entrada=identrada,comentarios=comentarios,form=FormularioComentario,usuario=request.user)
